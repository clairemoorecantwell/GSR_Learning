...
class richCand(candidate):
    def __init__(self, c, violations, observedProb, segsDict, segsList, segsOrder=None, activitys=None,
                 suprasegmentals=None, surfaceForm=None, operations=None):
...
class Grammar:
    ...
    def prepForLexC(self):
        print("Ready to learn with lexical indexation.  Maximum " + str(self.lexC_type) + " copies of each constraint.")
        ...
        if e:
            ...
            if self.p_useListed > 0:
                ...
                if random.random() < self.pToList:
                    self.trainingData.lexicon["_".join([i.tag for i in datum[0]])] = lexeme(
                        "_".join([i.tag for i in datum[0]]), [character for character in obs.surfaceForm])
            ##################################################################
...
class lexeme:
    def __init__(self, tag, segmentList=None, kind=None):
        ...
        self.segmentList = segmentList if segmentList else [i for i in
                                                            tag]  # Human-readable list of segments, corresponding to feature lookup tables, if using
        ...
        self.activitys = [1 for i in
                          self.segmentList]  # List of float value activity levels for self.segs.  Defaults to 1 for all
        self.linearSegOrder = [i for i in range(1,
                                                len(self.segmentList) + 1)]  # integers specifying the linear order of segs in self.segs. starts at 1
        # Example: t/z/n ami:
...
class trainingData:
    ...
    def __init__(self, filename):
        ...
        if 'input' in header:
            ...
            print(
                "ERROR: No column of your input file is labeled 'input'.  This column is required, and must be labelled exactly.  Please check your input file and try again")
            sys.exit()
        ...
        if 'obs.prob' in header:
            ...
            print(
                "ERROR: No column of your input file is labeled 'obs.prob'.  This column is required, and must be labelled exactly.  Please check your input file and try again")
            sys.exit()
        ...
        if 'surface' in header:
            ...
            if self.noisy:
                print(
                    "Your input file contains hidden structure!  Yay!  I'll try to fit that with Expectation-Maximization according to Jarosz (2013)")
        ...
        if 'lexeme' in header:
            ...
            if self.noisy:
                print(
                    "Your input file contains specially defined lexemes.  I'll use lexeme names from the 'input' column, but lexeme phonemes from the 'lexeme' column.")
        ...
        for i in lines[1:]:
            ...
            if hidden:
                ...
                print(
                    "ERROR: no column with surface forms in it.  Please add either a 'candidate' column, or a 'surface' column")
                sys.exit()
            if freq_weighted:
    ...
    while moarCandidates:
        for o in operations:
            for c in allCands:
                ...
                if random.random() < A / t or not (
                        containsObsOut):  # Continue to generate candidates until you generate the observed output
                    try:
